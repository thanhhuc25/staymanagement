 ...
 * @extends  Controller
 */
class Controller_Front_plan extends Controller_Common
{
  /**
  ...
  **/
  public function action_index()
  {
    // $sort = Input::get('sort');
    // if ($sort == '1') {
    //   $sort_option = " ORDER BY nokoriRMnum DESC";
    // }else if ($sort == 2) {
    ...
    Session::set('plan_search_option',$option);
  ...
  **/
  ...
  public function plan_list($data)
  {
    ...
    if ($flg_list['date'] != 'checked') {
      ...
      foreach ($plan_data as $key => $value) {
        foreach ($value['RTYPES'] as $k => $val) {
          ...
          $param['DATE'] = date('Y-m-d', strtotime($flg_list['staydate']));
          $result = $plan_rtype->get_plan_rtype_one_price_stock($param);

          if (isset($result['closing_day']) && isset($result['closing_time'])
           && date('Y-m-d', strtotime($param['DATE'].' '.$result['closing_day'].' day')).' '.date('H:i', strtotime($result['closing_time'].':00')) < date('Y-m-d H:i')
          ) {
            unset($plan_data[$key]);
          }elseif ($param['DATE'] < date('Y-m-d')) {
            unset($plan_data[$key]); // ここにくることじたいどうかとおもう
          }

          break; // 1プランにつき1タイプだけで良い（手仕舞いはプランに設定されているので）
        }
      }
    }
    ...
    $this->template->content = View_Smarty::forge('front/plan/list', $data);
  }
  ...
  **/
  public function action_detail($id)
  {
    ...
    if (isset($ids[6])) {
      $search_date = date('Y-m-d', strtotime($ids[6]));
    }else{
    ...
    for ($i='1'; $i <= '31' ; $i++) {
      ...
      if (Session::get('plan_search_option.ciDate') && Session::get('plan_search_option.ciDate') != 'on' && $result['class'] != 'bg01') {
        $color_date = date('Y-m-d', strtotime(str_replace(array('年','月','日'), array('-','-',''), Session::get('plan_search_option.ciDate'))));
        if ($param['DATE'] == $color_date) {
          $result['class'] = 'bg02';
        }
        for ($j = 1; $j < $date['stay_num']; $j++) {
          $color_date = date('Y-m-d', strtotime(str_replace(array('年','月','日'), array('-','-',''), Session::get('plan_search_option.ciDate'))) + (60*60*24*$j));
          if ($param['DATE'] == $color_date) {
            $result['class'] = 'bg02';
          }
        }
      ...
      if (isset($secret_data)) {
        $result['general_payment'] = $result['payment'];
        $result['payment'] -= round($result['payment'] * $secret_data['PLN_RATE'] / 100, 0, PHP_ROUND_HALF_EVEN);
      }
    ...
    if (count($ids) == 7) {
      ...
      $format = 'Ymd';
      $datec = DateTime::createFromFormat($format, $ids[6]);
      ...
      $reserve = array(
          ...
          'STAYRMNUM' => $ids[5],
          'STAYMDATENUM' => $ids[4],
          'CHECK_IN_STR' => $datec->format(__('date_format').'('.Config::get('staym.weeks.'.__('use_lang').'.'.$datec->format('w')).')'),
          'PRICE' => $convert_price_list, // つこうとらん
          'PRICE_SUM' => $sum, // つこうとらん
          'PRICE_TOTAL' => $sum * $ids[5], // つこうとらん
          ...
          'PAYMENT' => ($plan_rtype_data['PLN_CHG_PERSON'.$ids[3]] + $plan_rtype_data['PLN_CHG_EXCEPTION'.$ids[3]]) * $ids[3] * $ids[5],
          'URL'=> $ids[0].EXPLODE.$ids[1].EXPLODE.$ids[2].EXPLODE.$ids[3].EXPLODE.$ids[4].EXPLODE.$ids[5].EXPLODE.$ids[6],
        );

      // 残りの日数たす
      $remaining_day = $ids[4] - 1;
      while ($remaining_day > 0) {
        ...
        $reserve['BASIC_PAY'] += $plan_rtype_data['PLN_CHG_PERSON'.$ids[3]] + $plan_rtype_data['PLN_CHG_EXCEPTION'.$ids[3]];
        $reserve['PAYMENT'] += ($plan_rtype_data['PLN_CHG_PERSON'.$ids[3]] + $plan_rtype_data['PLN_CHG_EXCEPTION'.$ids[3]]) * $ids[3] * $ids[5];

        $remaining_day--;
      }

      $datec = DateTime::createFromFormat($format, date('Ymd', strtotime($ids[6].' + '.$reserve['STAYMDATENUM'].' day')));
      $reserve['CHECK_OUT_STR'] = $datec->format(__('date_format').'('.Config::get('staym.weeks.'.__('use_lang').'.'.$datec->format('w')).')');

      //$tpl = 'front/plan/detail_reserve'; //同じテンプレで処理
  ...
  **/
  public function action_recalculation()
  {
    ...
    $url = $_SERVER['HTTP_REFERER'];
    $chopped = explode('/', rtrim($url, '/'));

    $last_path = array_pop($chopped);
    if (strpos($last_path, 'index') !== false) {
      $last_path = array_pop($chopped);
    }

    $ids = explode('_', $last_path);
    if (count($ids) > 1) {
      $ids[5] = $date['rm_num']  ;
      $ids[4] = $date['stay_num'];
    }

    $url = implode('/', $chopped).'/'.implode(EXPLODE, $ids);
