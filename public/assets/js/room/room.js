$(function () {
  ...
  $(".stop").on('click', function(){
    ...
    data['htl_type_id'] = $(this).attr('htl_type_id');
    data['flg'] = '0';
 ...
 function sale_change(data){
  ...
  function sort_num_save(data){
    $.ajax({
        ...
        data: data,
        dataType: 'json',
        url : '/admin/room/disporderchange',
        success : function(result) {
            if(result == 1){
...
function new_room(){
    location.href = "/admin/room/new";
  }

function limit_change(){
    var limit = $('#limit').val();
    location.href = "/admin/room/limit/" + limit;
}
